...
import { y as defineComponent, z as ref, r as reactive, e as computed, C as watch, o as openBlock, i as createElementBlock, j as createBaseVNode, q as createVNode, w as withCtx, D as unref, F as Fragment, s as renderList, f as createBlock, m as createTextVNode, t as toDisplayString, l as createCommentVNode, M as withKeys, v as withModifiers, Z as createStaticVNode, Y as _sfc_main$5 } from "./index.3437b938.js";
import { _ as _sfc_main$4 } from "./VInput.bb7a011d.js";
import { _ as __unplugin_components_2 } from "./VControl.c459fb91.js";
import { _ as _sfc_main$2, a as _sfc_main$3 } from "./VOption.1954103c.js";
import { t as toFormValidator, m as mod } from "./index.1fbd2b5e.js";
import { a as useForm } from "./VLabel.c9d776c3.js";
import { C as CREAR_DOCUMENTO_REALTIME, R as REEMPLAZAR_DOCUMENTO_REALTIME, E as ELIMINAR_DOCUMENTO_REALTIME, L as LISTAR_DOCUMENTOS_REALTIME } from "./main.06195782.js";
import { A as Al, D as Da, I as Il, x as xa, E as El, w as firestore, K as Kh, Q as Qh, y as gl } from "./config.165bca36.js";
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: emits }) {
    ...
    const opcionesIntensidad = ref(GET_OPCIONES_INSIDAD_RUTINA(props.datosrutina.tipo_esfuerzo).valuesEntrenador);
    ...
    const validarNuevoBloqueComputed = computed(() => validarBloqueDistancia(datosNuevoBloque));
    ...
    watch(() => props.datosrutina.tipo_esfuerzo, () => {
      opcionesIntensidad.value = GET_OPCIONES_INSIDAD_RUTINA(props.datosrutina.tipo_esfuerzo).valuesEntrenador;
...
const CREAR_DOCUMENTO_FIRESTORE = async (rutaDocumento, datos) => {
  const resultDb = await Al(Da(firestore, rutaDocumento), datos);
  return resultDb.id;
};
const ACTUALIZAR_DOCUMENTO_FIRESTORE = async (colection, id, datos) => await Il(xa(firestore, colection, id), datos);
const ELIMINAR_DOCUMENTO_FIRESTORE = async (coleccion, id) => await El(xa(firestore, coleccion, id));
const CONSULTA_SIMPLE = async (coleccion, valor, condicion, resultado) => {
  const q = Kh(Da(firestore, coleccion), Qh(valor, condicion, resultado));
  const getConsulta = await gl(q);
  const datos = [];
...
const CONSULTA_COMPUESTA = async (coleccion, valor1, condicion1, resultado1, valor2, condicion2, resultado2) => {
  const q = Kh(Da(firestore, coleccion), Qh(valor1, condicion1, resultado1), Qh(valor2, condicion2, resultado2));
  const getConsulta = await gl(q);
  const datos = [];
  getConsulta.forEach((doc2) => {
    const obj = JSON.parse(JSON.stringify(doc2.data()));
  ...
  while (fecha < siguienteLunes) {
    semana.forEach((dia, index) => {
      if (fecha.getDay() === indexDiaSabado && dia.length !== 0 && index === idexDiaDomingo) {
        CREAR_RUTINA_PERSONALIZADA(__spreadProps(__spreadValues({}, dia), {
          fecha,
...
const LISTAR_RUTINAS_DEPORTISTA_ENTRENADOR = async (idDeportista, idEntrenador) => await CONSULTA_COMPUESTA("rutinas", "id_deportista", "==", idDeportista, "id_entrenador", "==", idEntrenador);
...
const ACTUALIZAR_RUTINA_PERSONALIZADA = async (idRutina, rutina) => {
  const rutinaGuardar = __spreadValues({}, rutina);
